...
```
kubeadm init --kubernetes-version=v1.28.15 --image-repository registry.aliyuncs.com/google_containers --pod-network-cidr=10.100.0.0/16  --service-cidr=10.200.0.0/16 --service-dns-domain=cluster.local --apiserver-advertise-address=192.168.3.201
```
## 配置kubeconfig文件
```
mkdir -p $HOME/.kube
